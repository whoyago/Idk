	...
	(
		...
		FROM (
			SELECT 
				...
				,MIN(DTVENCTO) OVER (PARTITION BY IdCliente) AS TITULO_MAIS_ANTIGO --MAX A PEDIDO DA TATI (2 EM ABERTO)
			...
			WHERE ([TipoRecebimento] is null)
				...
				AND DATEDIFF(DAY, DTVENCTO, GETDATE()) BETWEEN 1 AND 58
			) AS B
		WHERE RN = 1 		
	),
...
WHERE DATEDIFF(DAY, INADIMPLENTE.TITULO_MAIS_ANTIGO, GETDATE()) BETWEEN 1 AND 60
